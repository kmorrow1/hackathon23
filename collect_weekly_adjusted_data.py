...
def main():
    ...
    function_directory = os.path.join(data_directory, "timeSeries", "TIME_SERIES_WEEKLY_ADJUSTED")
    ...
    for symbol in symbols:
        ...
        api_url = f"{base_url}function=TIME_SERIES_WEEKLY_ADJUSTED&symbol={symbol}&apikey={os.environ.get('YOUR_API_KEY')}&datatype=csv"
        response = requests.get(api_url)
        time.sleep(WAIT_TIME)
        with open(os.path.join(symbol_directory, f"{symbol}_weekly_adjusted.csv"), 'wb') as file:
            file.write(response.content)

    print("Weekly adjusted data collection complete!")
