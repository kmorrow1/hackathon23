...
def main():
    ...
    function_directory = os.path.join(data_directory, "fundamental_data", "OVERVIEW")
    ...
    for symbol in symbols:
        ...
        api_url = f"{base_url}function=OVERVIEW&symbol={symbol}&apikey={os.environ.get('YOUR_API_KEY')}"
        response = requests.get(api_url)
        with open(os.path.join(symbol_directory, f"{symbol}_OVERVIEW.json"), "wb") as f:
            f.write(response.content)
        time.sleep(WAIT_TIME)
    print("Company overview data collection complete!")
