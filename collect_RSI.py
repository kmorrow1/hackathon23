...
FUNCTION = "RSI"
SAVE_PATH = "E:/stock_data/technical_indicators/RSI"
PERIODS = [7, 14, 21, 28]  # Common periods for RSI
...
def fetch_rsi_data(symbol, period):
...
def save_data(symbol, period, data):
    ...
    file_path = os.path.join(symbol_dir, f"{symbol}_rsi_period_{period}.csv")
...
if __name__ == "__main__":
    for symbol in symbols:
        for period in PERIODS:
            print(f"Fetching RSI data for {symbol} with period {period}...")
            data = fetch_rsi_data(symbol, period)
            if data:
    ...
    print("RSI data fetching completed.")
