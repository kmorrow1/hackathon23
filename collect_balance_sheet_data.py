...
def main():
    ...
    function_directory = os.path.join(data_directory, "fundamental_data", "BALANCE_SHEET")
    ...
    for symbol in symbols:
        ...
        api_url = f"{base_url}function=BALANCE_SHEET&symbol={symbol}&apikey={os.environ.get('YOUR_API_KEY')}"
        response = requests.get(api_url)
        with open(os.path.join(symbol_directory, f"{symbol}_BALANCE_SHEET.json"), "wb") as f:
            f.write(response.content)
        time.sleep(WAIT_TIME)

    print("Balance Sheet data collection complete!")
