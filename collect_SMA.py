...
FUNCTION = "SMA"
SAVE_PATH = "E:/stock_data/technical_indicators/SMA"
PERIODS = [7, 14, 21, 28]  # Common periods for SMA
...
def fetch_sma_data(symbol, period):
...
def save_data(symbol, period, data):
    ...
    file_path = os.path.join(symbol_dir, f"{symbol}_sma_period_{period}.csv")
...
if __name__ == "__main__":
    for symbol in symbols:
        for period in PERIODS:
            print(f"Fetching SMA data for {symbol} with period {period}...")
            data = fetch_sma_data(symbol, period)
            if data:
    ...
    print("SMA data fetching completed.")
