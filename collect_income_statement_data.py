...
def main():
    ...
    function_directory = os.path.join(data_directory, "fundamental_data", "INCOME_STATEMENT")
    ...
    for symbol in symbols:
        ...
        api_url = f"{base_url}function=INCOME_STATEMENT&symbol={symbol}&apikey={os.environ.get('YOUR_API_KEY')}"
        response = requests.get(api_url)
        with open(os.path.join(symbol_directory, f"{symbol}_INCOME_STATEMENT.json"), "wb") as f:
            f.write(response.content)
        time.sleep(WAIT_TIME)
    print("Income Statement data collection complete!")
