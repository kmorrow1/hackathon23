...
FUNCTION = "EMA"
SAVE_PATH = "E:/stock_data/technical_indicators/EMA"
PERIODS = [7, 14, 21, 28]  # Common periods for EMA
...
def fetch_ema_data(symbol, period):
...
def save_data(symbol, period, data):
    ...
    file_path = os.path.join(symbol_dir, f"{symbol}_ema_period_{period}.csv")
...
if __name__ == "__main__":
    for symbol in symbols:
        for period in PERIODS:
            print(f"Fetching EMA data for {symbol} with period {period}...")
            data = fetch_ema_data(symbol, period)
            if data:
    ...
    print("EMA data fetching completed.")
