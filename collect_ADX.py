...
FUNCTION = "ADX"
SAVE_PATH = "E:/stock_data/technical_indicators/ADX"
PERIODS = [7, 14, 21, 28]  # Common periods for ADX
...
def fetch_adx_data(symbol, period):
...
def save_data(symbol, period, data):
    ...
    file_path = os.path.join(symbol_dir, f"{symbol}_adx_period_{period}.csv")
...
if __name__ == "__main__":
    for symbol in symbols:
        for period in PERIODS:
            print(f"Fetching ADX data for {symbol} with period {period}...")
            data = fetch_adx_data(symbol, period)
            if data:
    ...
    print("ADX data fetching completed.")
