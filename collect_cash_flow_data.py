...
def main():
    ...
    function_directory = os.path.join(data_directory, "fundamental_data", "CASH_FLOW")
    ...
    for symbol in symbols:
        ...
        api_url = f"{base_url}function=CASH_FLOW&symbol={symbol}&apikey={os.environ.get('YOUR_API_KEY')}"
        response = requests.get(api_url)
        with open(os.path.join(symbol_directory, f"{symbol}_CASH_FLOW.json"), "wb") as f:
            f.write(response.content)
        time.sleep(WAIT_TIME)
    print("Cash Flow data collection complete!")
